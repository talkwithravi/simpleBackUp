...
func advertiseService() {
	...
	server, err := zeroconf.Register(
		"MacBackup-"+hostname, // service instance name
		"_simplebackup._tcp",  // service type
		"local.",              // domain
		port,                  // port
		[]string{"txtv=1", "app=mobile-backup", "version=1.0"}, // TXT record
		nil, // iface
	)
	if err != nil {
		log.Printf("Failed to register Bonjour service: %v", err)
		return
	}
	defer server.Shutdown()
	log.Printf("Bonjour service registered: _simplebackup._tcp.local:%d", port)
...
func main() {
	...
	// Start Bonjour/mDNS service advertisement in a goroutine
	go advertiseService()
